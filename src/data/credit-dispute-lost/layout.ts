...
const BG_HEIGHT = 1750 // covers 10 Y-slots (0–9) + 1 time chasm + header + padding
...
const FLOW_NODES: Node[] = [
  ...
  // ── Return path (yellow) ───────────────────────────────────────────────────
  {
    id: 'dispute-lost-stripe',
    type: 'paymentNode',
    position: { x: NODE_X.stripe, y: Y(8) },
    data: { label: 'Dispute Lost', sublabel: 'Issuer rules for cardholder, funds kept', stepIndex: 7, icon: 'server', timing: '~instant' },
  },
  {
    id: 'funds-retained',
    type: 'paymentNode',
    position: { x: NODE_X.customer, y: Y(9) },
    data: { label: 'Cardholder Wins', sublabel: 'Funds permanently returned to cardholder', stepIndex: 8, icon: 'monitor', timing: '~1–2 days' },
  },
]
...
export const EDGES: Edge[] = [
  ...
  { id: 'e6-7',   source: 'issuer-review',        target: 'dispute-lost-stripe',  label: 'decision: lost' },
  { id: 'e7-8',   source: 'dispute-lost-stripe',  target: 'funds-retained',       label: 'funds returned' },
]
